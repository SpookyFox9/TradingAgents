...
def _print_tax_loss_status(holding, portfolio, prices: dict) -> None:
    ...
    try:
        ...
        logger.warning("Spike check failed for %s: %s", holding.ticker, exc)
...
def _run_holdings_loop(ta, portfolio, run_holdings, prices, macro_snapshot, run_cfg, results, skipped):
    from portfolio_lib.loader import iter_holdings
    ...
    from portfolio_lib.signal_log import tag_compliance_block
    ...
    for holding in iter_holdings(portfolio):
        ...
        from tradingagents.dataflows.config import set_config as _set_ta_config
        _set_ta_config({"extra_instrument_context": extra_ctx})
        try:
            ...
            report_path = write_ticker_report(
                result, run_cfg.results_dir, run_cfg.analysis_date, run_cfg.run_timestamp,
                deep_mode=run_cfg.deep_mode, analyst_preset=run_cfg.analyst_preset,
            )
...
def _run_watchlist_loop(ta, portfolio, run_watchlist, prices, macro_snapshot, run_cfg, results, skipped):
    from portfolio_lib.loader import iter_watchlist
    from portfolio_lib.analyzer import analyze_ticker, TickerKind
    from portfolio_lib.reporter import write_ticker_report
    from portfolio_lib.executor import stage_pending_order
    from portfolio_lib.signal_log import tag_compliance_block
...
def _run_discovery_pass(ta, portfolio, args, prices, macro_snapshot, run_cfg, base_llm_config, results, skipped, all_candidates, survivors, near_misses):
    from portfolio_lib.loader import iter_watchlist
    from portfolio_lib.analyzer import analyze_ticker, TickerKind
    from portfolio_lib.reporter import write_ticker_report
    ...
    tmp_candidates = suggest_tickers(
    ...
    all_candidates.extend(tmp_candidates)
    survivors.extend([c for c in tmp_candidates if c.passed])
    near_misses.extend([c for c in tmp_candidates if c.near_miss and not c.passed])
    if survivors:
...
def _run_finalize(results, all_candidates, skipped, portfolio, macro_snapshot, cost_tracker, run_cfg):
    from portfolio_lib.cost_tracker import append_cost_log
    ...
    if slack_webhook:
        try:
            post_slack_digest(
                results, macro_snapshot.regime, cost_tracker.total_usd,
                portfolio.cash_balance, run_cfg.analysis_date, slack_webhook,
                run_timestamp=run_cfg.run_timestamp,
            )
        except Exception as exc:
...
def main() -> None:
    ...
    from portfolio_lib.loader import load_portfolio, iter_watchlist, persist_watchlist_additions
    from portfolio_lib.prices import clear_cache, get_price
    ...
    from portfolio_lib.cost_tracker import CostTracker
    ...
    base_llm_config = {**run_cfg.llm_config}
    if doctrine:
        base_llm_config["doctrine_context"] = doctrine
    ...
    if run_holdings:
        _run_holdings_loop(ta, portfolio, run_holdings, prices, macro_snapshot, run_cfg, results, skipped)

    if run_watchlist:
        _run_watchlist_loop(ta, portfolio, run_watchlist, prices, macro_snapshot, run_cfg, results, skipped)

    # ── Discovery pass (optional) ──────────────────────────────────────────────
    if args.discover and (not args.watchlist or args.discover_only):
        _run_discovery_pass(
            ta, portfolio, args, prices, macro_snapshot, run_cfg, base_llm_config,
            results, skipped, all_candidates, survivors, near_misses,
        )
    ...
    _run_finalize(results, all_candidates, skipped, portfolio, macro_snapshot, cost_tracker, run_cfg)
